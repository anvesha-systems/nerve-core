...
fn search_query_lifecycle() {
    ...
    client.read_exact(&mut header_buf).expect("read response header");
...
fn multiple_requests_different_ids() {
    ...
    client.read_exact(&mut response).expect("read ping response");
    ...
    let mut header_buf = vec![0u8; 20];
    client.read_exact(&mut header_buf).expect("read search response");
